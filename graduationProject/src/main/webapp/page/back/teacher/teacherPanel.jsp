			...
			$('#teacher_persnol').tree({
				onClick: function(node){
					if ($('#tabs').tabs('exists',node.text)) { //判断该选项卡是否打开
						...
						if ('基本信息' == node.text) { /* 获取列的信息并进行比较 */
							addTab(node.text,'/graduation/page/back/teacher/teacherBaseInfo.jsp');
						} else if ('修改密码' == node.text) {
							addTab(node.text,'/graduation/page/back/teacher/teacherUpdatePass.jsp');
						}
					}
				}
			});
...
<body class="easyui-layout" style="overflow-y: hidden">
    ...
    <div id="mainPanle" region="center" style="background: #eee; overflow-y:hidden;background:url('/graduation/images/teacher.jpg');">
